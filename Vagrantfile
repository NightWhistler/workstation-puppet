...
Vagrant.configure(2) do |config|
  ...
  if ("#{ARGV[1]}"=="")
   ARGV[1]='xenial-puppet4'
  end

  config.vm.define "#{ARGV[1]}", primary: true do |standalone|
    standalone.vm.box = "dapas/xenial-puppet4-sf"
    standalone.vm.network "forwarded_port", guest: 443, host: 8443
    standalone.vm.network "forwarded_port", guest: 80, host: 8080
    standalone.vm.hostname="#{ARGV[1]}"
